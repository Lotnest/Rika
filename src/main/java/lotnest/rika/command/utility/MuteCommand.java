...
public class MuteCommand extends Command {
    ...
    public void execute(final @NotNull CommandInfo commandInfo) {
        ...
        if (MemberUtil.isModerator(commandInfo.getExecutor(), channel)) {
            ...
            if (commandInfo.getArgs().length < 2 || mentionedMembers.isEmpty()) {
                ...
                MemberUtil.findRoleAndAddToMember(MessageProperty.MUTED_ROLE_NAME, mentionedMembers.get(0), MessageProperty.USER_MUTED, MessageProperty.ROLE_NOT_FOUND, MessageProperty.USER_ALREADY_MUTED, embedBuilder, channel);
            }
